...
#
# DESCRIPTION:
# Connect to a CheckValve Console Relay and print messges from the specified game server.
#
# AUTHOR:
# Dave Parker
#
...
# Validate the Console Relay information
if( ! defined($cr) ) {
	...
	if( ($cr_port !~ m/^\d{1,5}$/) or ($cr_port > 65535) ) {
		print "\nERROR: The Console Relay port must be a number (1-65535).\n";
		usage();
		exit(1);
	}
}
...
connectToConsoleRelay();
getConsoleMessages();
exit(0);

sub connectToConsoleRelay {
	...
	$socket = IO::Socket::INET->new(
		...
		Type      => SOCK_STREAM,
		Blocking  => 1
	) or die "ERROR: Failed to connect to the CheckValve Console Relay server.\n($!)\n";
}

sub getConsoleMessages {
	my ($recv, $byte, $protocol, $response, $header, $message, $server_tstamp, $ip, $port, $tstamp);
	my @bytes;
	my @fields;
...
sub usage {
	print <<EOT
console_logger.pl version $PROGRAM_VERSION

Usage: $prog [--console-relay <ip>:<port>] [--game-server <ip>:<port>] [--limit <num>] [--password <password>] [--quiet]
       $prog [--help]

Command line options:
    -c|--console-relay <ip>:<port>  Connect to the CheckValve Console Relay at the specified IP and port (required).
    -g|--game-server <ip>:<port>    Request console messages from the game server at the specified IP and port (required).
    -l|--limit <num>                Stop the console logger after receiving <num> messages.
    -p|--password <password>        Specify the CheckValve Console Relay password.
